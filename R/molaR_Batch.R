...
molaR_Batch <- function(pathname=getwd(), DNE=TRUE, RFI=TRUE, OPCr=TRUE, OPC=FALSE, Details=FALSE,
  ...
  ###FIX THIS###
  R1_Output <- vector('numeric')
  R2_Output <- vector('numeric')
  R3_Output <- vector('numeric')
  R4_Output <- vector('numeric')
  R5_Output <- vector('numeric')
  R6_Output <- vector('numeric')
  R7_Output <- vector('numeric')
  R8_Output <- vector('numeric')
  OPC_Output <- vector('numeric')
  if(Details==TRUE){
    ThreeD_Area <- vector('numeric')
    TwoD_Area <- vector('numeric')
    RotDegrees <- seq(0, (OPCr_steps*OPCr_stepSize)-OPCr_stepSize, OPCr_stepSize)
    Rotations <- matrix(nrow=length(fileNames), ncol=length(RotDegrees))
    colnames(Rotations) <- paste(as.character(RotDegrees), "deg.")
  }
  for(i in 1:fileNum){
    invisible(capture.output(Specimen <- vcgPlyRead(file.path(pathname, fileNames[i]))))
    if(DNE==TRUE){
      invisible(capture.output({DNE_Specimen <- DNE(Specimen, outliers=DNE_outliers)}))
    ...
    if(OPCr==TRUE){
      ...
      invisible(capture.output({OPCr_Specimen <- OPCr(Specimen, Steps=OPCr_steps, 
                                                      stepSize=OPCr_stepSize, 
                                                      minimum_faces=OPCr_minimum_faces, 
                                                      minimum_area=OPCr_minimum_area)}))
      OPCr_Result <- OPCr_Specimen$OPCR
      OPCr_Output <- c(OPCr_Output, OPCr_Result)
        if(Details==TRUE){
          for(j in 1:OPCr_steps){
            Rotations[i,j] <- OPCr_Specimen$Each_Run[[j,2]]
          }
        }
    }
    if(OPC==TRUE){
      invisible(capture.output({OPC_Specimen <- OPC(Specimen, rotation=OPC_rotation, 
                                                    minimum_faces=OPC_minimum_faces, 
                                                    minimum_area=OPC_minimum_area)}))
  ...
  if(RFI==TRUE){
    ...
    if(Details==TRUE){
      Output <- cbind(Output, '3D_Area'=ThreeD_Area, '2D_Area'=TwoD_Area)
    }
  }
  if(OPCr==TRUE){
  ...
  print(Output)
  write.csv(Output, file=file.path(pathname, "molaR_Batch.csv"), row.names=FALSE)
